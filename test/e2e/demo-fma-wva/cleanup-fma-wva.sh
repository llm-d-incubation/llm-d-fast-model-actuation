...
if [ "$FULL_CLEANUP" = "true" ]; then
    ...
    # WVA's install.sh defaults DEPLOY_PROMETHEUS_ADAPTER=true, which on undeploy
    # means it tries to `helm uninstall` an adapter that this demo never
    # installed (OpenShift's user-workload monitoring handles metrics here).
    # Force both Prometheus-related flags to false so the undeploy is symmetric
    # with what the demo script deployed and we don't emit misleading
    # "uninstalling adapter" warnings from a no-op delete.
